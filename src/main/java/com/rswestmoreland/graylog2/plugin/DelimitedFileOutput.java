 ...
 * 
 * The SyslogOutput Plugin (by Huksley and TCollins) was used as the initial
 * framework I still have come cleanup, refactoring, and commenting to do.
 * 
...
public class DelimitedFileOutput implements MessageOutput {
	...
	public String endlineSequence;
	public String filepath;
	public String filedone;
	public String fields;
	public String[] fieldList;
	public int fieldListSize = 0;
	...
	public MessageSender createSender(String format) {
		switch (format) {
		case "csv":
			return new FormatCSV(fileWrite, this);
		case "pipe":
			return new FormatPipe(fileWrite, this);
		case "space":
			return new FormatSpace(fileWrite, this);
		case "tsv":
			return new FormatTSV(fileWrite, this);
		default:
			return new FormatCSV(fileWrite, this);
		}
	}
	...
	@Inject
	public DelimitedFileOutput(@Assisted Stream stream, @Assisted Configuration config, NodeId nodeId)
		...
		fileWrite = new FileWrite(this);
		fileWrite.NewFile();

		if (rotateinterval != 0) {
			if (strategy.equals("interval")) {
				scheduler.scheduleAtFixedRate(fileWrite.PeriodicRotate, rotateinterval, rotateinterval,
						TimeUnit.SECONDS);
			} else {
				countLines = true;
			}
	...
	public static class Config extends MessageOutput.Config {
		...
		public ConfigurationRequest getRequestedConfiguration() {
			...
			configurationRequest.addField(new DropdownField("fileformat", "File Format", "csv", formats,
			...
			configurationRequest.addField(new TextField("filepath", "File Path", DEFAULTPATH,
					"File path to write messages to.  Available substitution variables are $HOST, $NODE, $EPOCH, $PID, $THREAD, $ROTATE, $PADDED.",
					ConfigurationField.Optional.NOT_OPTIONAL));
			configurationRequest
					.addField(new TextField("fields", "Fields", DEFAULTFIELDS, "Comma separated fields to export.",
			...
			configurationRequest.addField(new DropdownField("endline", "End of Line", "newline", endlines,
					"The special characters used by systems to represent end of line.",
					ConfigurationField.Optional.NOT_OPTIONAL));

			Map<String, String> compresslist = ImmutableMap.of("none", "None", "gzip", "GZIP", "gzip_fast",
					"GZIP Fastest", "gzip_max", "GZIP Max Compression");
			configurationRequest.addField(new DropdownField("compress", "Compression Options", "none", compresslist,
					"Optionally compress the file in realtime.", ConfigurationField.Optional.NOT_OPTIONAL));

			Map<String, String> strategies = ImmutableMap.of("interval", "Interval", "count", "Count");
			configurationRequest.addField(new DropdownField("strategy", "Rotation Strategy", "interval", strategies,
					"How the output filename will change to prevent unlimited growth.",
					ConfigurationField.Optional.NOT_OPTIONAL));

			configurationRequest.addField(new NumberField("rotateinterval", "Rotate Counter", DEFAULTROTATEINT,
					"Seconds or line count until file rotation, depending on selected strategy.  Disable by setting to zero.",
					ConfigurationField.Optional.NOT_OPTIONAL, NumberField.Attribute.ONLY_POSITIVE));
			configurationRequest.addField(new TextField("filedone", "Append Extension", DEFAULTDONE,
					"Append file with extension when rotating.", ConfigurationField.Optional.OPTIONAL));

			configurationRequest.addField(new NumberField("buffersize", "Buffer Size", DEFAULTBUFFER,
					"Write buffer in bytes. Must be greater than zero, multiple of 1024 recommended.",
					ConfigurationField.Optional.NOT_OPTIONAL, NumberField.Attribute.ONLY_POSITIVE));
			configurationRequest.addField(new NumberField("flushinterval", "Flush Interval", DEFAULTINTERVAL,
					"Seconds to flush write buffer.  Disable by setting to zero.",
					ConfigurationField.Optional.NOT_OPTIONAL, NumberField.Attribute.ONLY_POSITIVE));

			configurationRequest.addField(
					new BooleanField("debug", "Debug", false, "Enable debugging to troubleshoot file writes."));
